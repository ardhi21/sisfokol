...
if ($_POST['btnUPL'])
	...
	if (empty($filex_namex))
		...
		if ($ext_filex == ".jpg")
			{
			//nilai
			$filex1 = "../../../filebox/e/artikel/$artkd/$filex_namex";

			//cek, sudah ada belum
			if (!file_exists($filex1))
				{
				//mengkopi file
				move_uploaded_file($_FILES['filex']['tmp_name'],"../../../filebox/e/artikel/$artkd/$filex_namex");

				//query
				mysql_query("INSERT INTO guru_mapel_artikel_filebox(kd, kd_guru_mapel_artikel, filex) VALUES ".
								"('$x', '$artkd', '$filex_namex')");
...
//koleksi file
$qfle = mysql_query("SELECT * FROM guru_mapel_artikel_filebox ".
						"WHERE kd_guru_mapel_artikel = '$artkd' ".
						"ORDER BY filex ASC");
...
if ($tfle != 0)
	...
	do
		...
		echo '* <input name="filex'.$nomer.'" type="text" value="'.$sumber.'/filebox/e/artikel/'.$artkd.'/'.$fle_filex.'" size="75" readonly="true">';
