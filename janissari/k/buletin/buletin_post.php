...
//nilai
$filenya = "buletin_post.php";
$judul = "Entry/Edit Buletin";
...
if ($_POST['btnSMP'])
	...
	if ((empty($kategorix)) OR (empty($judulx)) OR (empty($rangkumanx)) OR (empty($isix)))
		...
		$ke = "buletin_post.php?s=baru&bulkd=$bulkd";
		pekem($pesan,$ke);
		exit();
		}
	else
		...
		if ($s == "baru")
			{
			//cek
			$qcc = mysql_query("SELECT * FROM user_blog_buletin ".
			...
			if ($tcc != 0)
				{
				//re-direct
				$pesan = "Buletin Tersebut Sudah Ada. Silahkan Ganti Yang Lain...!!";
				$ke = "$filenya?s=baru";
				pekem($pesan,$ke);
				exit();
				}
				...
				{
				//insert
				mysql_query("INSERT INTO user_blog_buletin(kd, kd_user, kd_kategori, judul, rangkuman, isi, postdate) VALUES ".
								"('$bulkd', '$kd1_session', '$kategorix', '$judulx', '$rangkumanx', '$isix', '$today')");
		...
		if ($s == "edit")
			...
			if ($tcc > 1)
				{
				//re-direct
				$pesan = "Ditemukan Duplikasi Buletin Tersebut. Silahkan Ganti Yang Lain...!!";
				$ke = "$filenya?s=edit&bulkd=$bulkd";
				pekem($pesan,$ke);
				exit();
				}
			else
				{
				//update
				mysql_query("UPDATE user_blog_buletin SET kd_kategori = '$kategorix', ".
								"judul = '$judulx', ".
...
<tr>
<td>
<strong>Isi Buletin : </strong>
</td>
<td align="right">
<input name="btnUPL" type="button" value="FileBox Image >>"OnClick="javascript:MM_openBrWindow(\'buletin_post_filebox.php?bulkd='.$bulkd.'\',\'FileBOX Image (.jpg) :\',\'width=650,height=300,toolbar=no,menubar=no,location=no,scrollbars=yes,resize=no\')">
</td>
</tr>
</table>
