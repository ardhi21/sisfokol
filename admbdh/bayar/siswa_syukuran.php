...
//nilai
$filenya = "siswa_syukuran.php";
$judul = "Keuangan Siswa : Uang Syukuran";
...
if ($s == "hapus")
	...
	//query
	mysql_query("DELETE FROM siswa_uang_syukuran ".
...
if ($_POST['btnSMP'])
	...
	//yang telah dibayar
	$qcc = mysql_query("SELECT SUM(nilai) AS nilai FROM siswa_uang_syukuran ".
				"WHERE kd_tapel = '$tapelkd' ".
				"AND kd_siswa = '$swkd'");
	...
	if (empty($nil_bayar))
		...
		if (($cc_nilai + $nil_bayar) > $pkl_nilai)
			{
			//re-direct
			$pesan = "Jumlah Uang Syukuran Yang Dibayarkan, Melebihi Jumlah Total Sisa Yang Ada. Harap Diperhatikan...!!";
			$ke = "$filenya?tapelkd=$tapelkd&nis=$nis";
			pekem($pesan,$ke);
			exit();
			}
		...
		//jika kurang
			...
			{
			mysql_query("INSERT INTO siswa_uang_syukuran (kd, kd_tapel, kd_siswa, tgl_bayar, nilai, postdate) VALUES ".
					"('$x', '$tapelkd', '$swkd', '$today', '$nil_bayar', '$today3')");


			//re-direct
			$ke = "siswa_syukuran_prt.php?tapelkd=$tapelkd&nis=$nis";
			xloc($ke);
			exit();
			}
		}
