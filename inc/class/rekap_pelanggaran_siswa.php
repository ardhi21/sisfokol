...
function Header()
{
	...
	//kelas sebelumnya
	$xkelx_nox = round($rkel_no - 1);
	$qkx = mysql_query("SELECT * FROM m_kelas ".
				"WHERE no = '$xkelx_nox'");
	$rkx = mysql_fetch_assoc($qkx);
	$rkelx_kd = nosql($rkx['kd']);
	$rkelx = nosql($rkx['kelas']);
	...
	//query
	$qccx = mysql_query("SELECT * FROM m_siswa ".
				"WHERE kd = '$swkd'");
	$rccx = mysql_fetch_assoc($qccx);
	$tccx = mysql_num_rows($qccx);
	$e_nis = nosql($rccx['nis']);
	$e_nama = balikin($rccx['nama']);


	//walikelas
	$qduk = mysql_query("SELECT m_walikelas.*, m_pegawai.* ".
	...
	$rduk = mysql_fetch_assoc($qduk);
	$duk_nama = balikin($rduk['nama']);


    	$this->SetFont('Times','',12);

	$this->SetY(10);
	$this->Cell(50,5,'Nama Siswa ',0,1,'L');

	$this->SetY(15);
	$this->Cell(50,5,'Kelas',0,1,'L');

	$this->SetY(20);
	$this->Cell(50,5,'Wali Kelas',0,1,'L');

	$this->SetY(25);
	$this->Cell(50,5,'Jumlah Skor Selama Kelas '.$rkelx.'',0,1,'L');


	$this->SetY(10);
	$this->SetX(70);
	$this->Cell(80,5,': '.$e_nis.'. '.$e_nama.'',0,1,'L');

	$this->SetY(15);
	$this->SetX(70);
	$this->Cell(80,5,': '.$rkel.'',0,1,'L');

	$this->SetY(20);
	$this->SetX(70);
	$this->Cell(80,5,': '.$duk_nama.'',0,1,'L');

	$this->SetY(25);
	$this->SetX(70);
	$this->Cell(80,5,': '.$rkox_pot.'',0,1,'L');
	}
