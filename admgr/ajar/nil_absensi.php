...
if ($_POST['btnHPS'])
	...
	//re-direct
	$ke = "$filenya?tapelkd=$tapelkd&keahkd=$keahkd&kelkd=$kelkd&smtkd=$smtkd&progkd=$progkd";
	xloc($ke);
	exit();
	}
...
if ($_POST['btnSMP'])
	...
	//re-direct
	$ke = "$filenya?tapelkd=$tapelkd&keahkd=$keahkd&kelkd=$kelkd&rukd=$rukd&".
...
do
	...
	echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&keahkd='.$keahkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&progkd='.$progkd.'&smtkd='.$st_kd.'">'.$st_smt.'</option>';
	}
...
for ($i=1;$i<=31;$i++)
	...
	echo'<option value="'.$filenya.'?tapelkd='.$tapelkd.'&keahkd='.$keahkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&progkd='.$progkd.'&smtkd='.$smtkd.'&pertemuan='.$i.'&utgl='.$cc_tgl.'&ubln='.$cc_bln.'&uthn='.$cc_thn.'">'.$i.' ['.$cc_tgl.' '.$arrbln[$cc_bln].''.$cc_thn.']</option>';
	}
...
for ($itgl=1;$itgl<=31;$itgl++)
	{
	echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&keahkd='.$keahkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&smtkd='.$smtkd.'&pertemuan='.$pertemuan.'&utgl='.$itgl.'">'.$itgl.'</option>';
	}
echo '</select>';
...
for ($i=1;$i<=12;$i++)
	...
	if ($i<=6) //bulan juli sampai desember
		...
		echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&keahkd='.$keahkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&smtkd='.$smtkd.'&pertemuan='.$pertemuan.'&utgl='.$utgl.'&ubln='.$ibln.'&uthn='.$tpx_thn1.'">'.$arrbln[$ibln].' '.$tpx_thn1.'</option>';
		}
		...
		echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&keahkd='.$keahkd.'&kelkd='.$kelkd.'&progkd='.$progkd.'&smtkd='.$smtkd.'&pertemuan='.$pertemuan.'&utgl='.$utgl.'&ubln='.$ibln.'&uthn='.$tpx_thn2.'">'.$arrbln[$ibln].' '.$tpx_thn2.'</option>';
		}
	}
...
if (empty($smtkd))
	...
	$pages = $p->findPages($count, $limit);
	$result = mysql_query("$sqlresult LIMIT ".$start.", ".$limit);
	$target = "$filenya?tapelkd=$tapelkd&keahkd=$keahkd&kelkd=$kelkd&".
