...
if ($count != 0)
	...
	do 
		...
		//stock pinjam
		$qpjt = mysql_query("SELECT * FROM perpus_stock ".
								"WHERE kd_item = '$i_kd'");
		$rpjt = mysql_fetch_assoc($qpjt);
		$pjt_bagus = nosql($rpjt['jml_bagus']);
		$pjt_dipinjam = nosql($rpjt['jml_dipinjam']);
		$pjt_sisa = round($pjt_bagus - $pjt_dipinjam);
		
		
		//status
		if ($i_status == "false")
			{
			$i_status_ket = "<font color=\"blue\">Belum Bisa Dipinjam</font>";
			}
		else
			{
			$i_status_ket = "<font color=\"brown\">BISA DIPINJAM</font>";
			}
		...
		<br>
		***Sisa Stock Pinjam : 
		<strong><strong>'.$pjt_sisa.'</strong></strong> 
