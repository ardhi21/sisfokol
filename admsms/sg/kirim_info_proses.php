	...
	if (($_POST['btnKRM']) OR (!$swkd))
		...
		if ($tdata != 0)
			...
			if (($smkux_nis < $smkux2_nis) AND ($s == "aktif"))
				{
				echo '<script>
				setTimeout("location.href=\''.$filenya.'?tapelkd='.$tapelkd.'&keakd='.$keakd.'&kelkd='.$kelkd.'&swkd='.$i_swkd.'&swnis='.$i_swnis.'&s=aktif\'", '.$delay_detik.'000);
				</script>';
			...
			//siswa terakhir...
				...
				{
				echo '<script>
				setTimeout("location.href=\''.$filenya.'?tapelkd='.$tapelkd.'&keakd='.$keakd.'&kelkd='.$kelkd.'&s=selesai\'", '.$delay_detik.'000);
				</script>
