...
$sqlcount = "SELECT m_user.*, user_blog_note.*, user_blog_note.kd AS stkd ".
				"FROM m_user, user_blog_note ".
				"WHERE user_blog_note.kd_user = m_user.kd ".
				"AND user_blog_note.kd_user = '$uskd' ".
				"ORDER BY user_blog_note.postdate DESC";
$sqlresult = $sqlcount;
...
if ($count != 0)
	...
	do
		...
		[<a href="'.$filenya.'?uskd='.$uskd.'&page='.$page.'&bk=note&stkd='.$dt_kd.'#'.$dt_kd.'" title="Beri Komentar">Beri Komentar</a>].';
	...
	{
	echo '<br>
	<font color="red"><strong>TIDAK ADA DAFTAR NOTE.</strong></font>';
	}
