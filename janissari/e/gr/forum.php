...
if (($s == "hapus") AND (!empty($stkd)))
	{
	//hapus topik
	mysql_query("DELETE FROM guru_mapel_forum ".
					"WHERE kd_guru_mapel = '$gmkd' ".
					"AND kd = '$stkd'");

	//hapus msg
	mysql_query("DELETE FROM guru_mapel_forum_msg ".
...
if ($_POST['btnSMP'])
	...
	if (empty($topik))
		...
		{
		//query
		mysql_query("INSERT INTO guru_mapel_forum(kd, kd_guru_mapel, topik, postdate) VALUES ".
						"('$x', '$gmkd', '$topik', '$today')");
...
//nek mapel telah dipilih
	...
	if ($tpel != 0)
		...
		<td>
		Topik Baru :
		<input name="topik" type="text" value="" size="50" '.$x_enter.'>
		...
		if ($count != 0)
			...
			do
				...
				[<a href="'.$filenya.'&page='.$page.'&s=hapus&stkd='.$i_fkd.'" title="HAPUS"><img src="'.$sumber.'/img/delete.gif" width="16" height="16" border="0"></a>].';
