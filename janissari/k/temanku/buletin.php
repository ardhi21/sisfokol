...
$sqlcount = "SELECT m_user.*, user_blog_teman.*, ".
				"user_blog_buletin.*, user_blog_buletin.kd AS stkd ".
				"FROM m_user, user_blog_teman, user_blog_buletin ".
				"WHERE user_blog_buletin.kd_user = m_user.kd ".
				"AND user_blog_teman.kd_user_teman = m_user.kd ".
				"AND user_blog_teman.kd_user = '$kd1_session' ".
				"ORDER BY user_blog_buletin.postdate DESC";
$sqlresult = $sqlcount;
...
if ($count != 0)
	...
	do
		...
		[<em>'.$dt_postdate.'</em>].
		<br>
		[<a href="'.$sumber.'/janissari/p/buletin/buletin.php?uskd='.$dt_uskd.'&bulkd='.$dt_kd.'" title="'.$dt_judul.'">Selengkapnya...</a>].
		[<a href="'.$filenya.'?page='.$page.'&s=view&stkd='.$dt_kd.'#'.$dt_kd.'" title="('.$titusx.') Komentar">(<strong>'.$titusx.'</strong>) Komentar</a>].
		[<a href="'.$filenya.'?page='.$page.'&bk=buletin&stkd='.$dt_kd.'#'.$dt_kd.'" title="Beri Komentar">Beri Komentar</a>]. ';
		...
		//jika beri komentar
		else if (($bk == "buletin") AND ($stkd == $dt_kd))
			{
			//jika batal
			if ($_POST['btnBTL'])
				{
			...
			if ($_POST['btnSMP'])
				...
				mysql_query("INSERT INTO user_blog_buletin_msg(kd, kd_user_blog_buletin, dari, msg, postdate) VALUES ".
								"('$x', '$stkd', '$kd1_session', '$bk_buletin', '$today')");
	...
	{
	echo '<br>
	<font color="red"><strong>TIDAK ADA DAFTAR BULETIN TEMAN.</strong></font>';
	}
