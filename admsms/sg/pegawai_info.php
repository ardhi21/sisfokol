...
if ($_POST['btnSMP'])
	...
	if (empty($info))
		...
		{
		///cek
		$qcc = mysql_query("SELECT * FROM sms_pegawai_info");
		...
		if ($tcc != 0)
			{
			//query
			mysql_query("UPDATE sms_pegawai_info SET info = '$info', ".
					"postdate = '$today'");
			}
		else
			{
			//query
			mysql_query("INSERT INTO sms_pegawai_info(kd, info, postdate) VALUES ".
					"('$x', '$info', '$today')");
			}
