...
function Header()
{
	...
	//smt
	$qtp2 = mysql_query("SELECT * FROM m_smt ".
				"WHERE kd = '$smtkd'");
	$rtp2 = mysql_fetch_assoc($qtp2);
	$smt = nosql($rtp2['smt']);
	...
	if ($this->PageNo() == 1)
		...
		$this->GetY();
		$this->Cell(280,5,'LAPORAN MUTASI BARANG PERSEDIAAN',0,1,'C');
		$this->GetY()+7;
		$this->Cell(280,5,'TAHUN PELAJARAN : '.$tapel.'',0,1,'C');
		$this->GetY()+7;
		$this->Cell(280,5,'SEMESTER : '.$smt.'',0,1,'C');
		$this->Ln();
		}
	}
