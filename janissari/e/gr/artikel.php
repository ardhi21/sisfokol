...
if (($s == "hapus") AND (!empty($artkd)))
	...
	//del data
	mysql_query("DELETE FROM guru_mapel_artikel ".
					"WHERE kd = '$artkd'");


	//query
	$qcc = mysql_query("SELECT * FROM guru_mapel_artikel_filebox ".
	...
	do
		...
		//hapus file
		$cc_filex = $rcc['filex'];
		$path1 = "../../../filebox/e/artikel/$artkd/$cc_filex";
		unlink ($path1);
		}
	while ($rcc = mysql_fetch_assoc($qcc));

	//hapus query
	mysql_query("DELETE FROM guru_mapel_artikel_filebox ".
					"WHERE kd_guru_mapel_artikel = '$artkd'");
...
if ($_POST['btnSMPx'])
	...
	if ((empty($kategorix)) OR (empty($judulx)) OR (empty($rangkumanx)) OR (empty($isix)))
		...
		if ($s == "baru")
			{
			//cek
			$qcc = mysql_query("SELECT guru_mapel.*, guru_mapel_artikel.*, guru_mapel_artikel.kd AS nkd ".
									...
									"AND guru_mapel_artikel.kd_kategori = '$kategorix' ".
									"AND guru_mapel_artikel.judul = '$judulx'");
			$rcc = mysql_fetch_assoc($qcc);
			...
			if ($tcc != 0)
				...
				{
				//insert
				mysql_query("INSERT INTO guru_mapel_artikel(kd, kd_guru_mapel, kd_kategori, judul, rangkuman, isi, postdate) VALUES ".
		...
		if ($s == "edit")
			{
			//update
			mysql_query("UPDATE guru_mapel_artikel SET kd_kategori = '$kategorix', ".
							"judul = '$judulx', ".
...
//nek mapel telah dipilih
	...
	if ($tpel != 0)
		...
		//nilai
		$filenya = "artikel.php?gmkd=$gmkd";
		$judul = "E-Learning : $pel_nm --> Artikel";
		...
		if (($s == "edit") OR ($s == "baru"))
			...
			//nilai
			//query
			$qdt = mysql_query("SELECT guru_mapel.*, guru_mapel_artikel.*, guru_mapel_artikel.kd AS nkd ".
									"FROM guru_mapel, guru_mapel_artikel ".
									"WHERE guru_mapel_artikel.kd_guru_mapel = guru_mapel.kd ".
									"AND guru_mapel.kd_user = '$kd1_session' ".
									"AND guru_mapel.kd = '$gmkd' ".
									"AND guru_mapel_artikel.kd = '$artkd'");
			...
			<td align="right">
			<input name="btnUPL" type="button" value="FileBox Image >>"OnClick="javascript:MM_openBrWindow(\'artikel_filebox.php?artkd='.$artkd.'\',\'FileBOX Image (.jpg) :\',\'width=650,height=300,toolbar=no,menubar=no,location=no,scrollbars=yes,resize=no\')">
			</td>
			</tr>
			</table>
		...
		//jika daftar //////////////////////////////////////////////////////////////////////////////////
			...
			if ($count != 0)
				...
				do
					...
					if (($s == "view") AND ($artkd == $dt_kd))
						...
						if ($a == "hapus")
							{
							//hapus
							mysql_query("DELETE FROM guru_mapel_artikel_msg ".
											"WHERE kd_guru_mapel_artikel = '$artkd' ".
											"AND kd = '$msgkd'");
					...
					//jika beri komentar
						...
						if ($_POST['btnSMP'])
							...
							//query
							mysql_query("INSERT INTO guru_mapel_artikel_msg(kd, kd_guru_mapel_artikel, dari, msg, postdate) VALUES ".
				...
				{
				echo '<font color="blue"><strong>TIDAK ADA Artikel. Silahkan Entry Dahulu...!!</strong></font>';
				}
