...
$sqlcount = "SELECT m_user.*, user_blog_teman.*, ".
				"user_blog_artikel.*, user_blog_artikel.kd AS stkd ".
				"FROM m_user, user_blog_teman, user_blog_artikel ".
				"WHERE user_blog_artikel.kd_user = m_user.kd ".
				"AND user_blog_teman.kd_user_teman = m_user.kd ".
				"AND user_blog_teman.kd_user = '$kd1_session' ".
				"ORDER BY user_blog_artikel.postdate DESC";
$sqlresult = $sqlcount;
...
if ($count != 0)
	...
	do
		...
		[<em>'.$dt_postdate.'</em>].
		<br>
		[<a href="'.$sumber.'/janissari/p/artikel/artikel.php?uskd='.$dt_uskd.'&artkd='.$dt_kd.'" title="'.$dt_judul.'">Selengkapnya...</a>].
		[<a href="'.$filenya.'?page='.$page.'&s=view&stkd='.$dt_kd.'#'.$dt_kd.'" title="('.$titusx.') Komentar">(<strong>'.$titusx.'</strong>) Komentar</a>].
		[<a href="'.$filenya.'?page='.$page.'&bk=artikel&stkd='.$dt_kd.'#'.$dt_kd.'" title="Beri Komentar">Beri Komentar</a>]. ';
		...
		//jika beri komentar
		else if (($bk == "artikel") AND ($stkd == $dt_kd))
			{
			//jika batal
			if ($_POST['btnBTL'])
				{
			...
			if ($_POST['btnSMP'])
				...
				mysql_query("INSERT INTO user_blog_artikel_msg(kd, kd_user_blog_artikel, dari, msg, postdate) VALUES ".
								"('$x', '$stkd', '$kd1_session', '$bk_artikel', '$today')");
	...
	{
	echo '<br>
	<font color="red"><strong>TIDAK ADA DAFTAR ARTIKEL TEMAN.</strong></font>';
	}
