...
if ($s == "edit")
	...
	//query
	$qx = mysql_query("SELECT * FROM perpus_supplier ".
						"WHERE kd = '$kdx'");
...
if ($_POST['btnSMP'])
	...
	if ($s == "baru")
		...
		if ((empty($nama)) OR (empty($alamat)) OR (empty($email)) OR (empty($telepon)) OR (empty($situs)))
			...
			{
			///cek
			$qcc = mysql_query("SELECT * FROM perpus_supplier ".
									"WHERE nama = '$nama'");
			...
			if ($tcc != 0)
				...
				//re-direct
				$pesan = "Nama Supplier : $nama, Sudah Ada. Silahkan Ganti Yang Lain...!!";
				$ke = "$filenya?s=baru";
				pekem($pesan,$ke);
				exit();
				}
				...
				{
				//query
				mysql_query("INSERT INTO perpus_supplier(kd, nama, alamat, email, telepon, situs) VALUES ".
	...
	//jika update
		...
		if (($tcc != 0) AND ($cc_kd != $kd))
			...
			{
			//query
			mysql_query("UPDATE perpus_supplier SET nama = '$nama', ".
							"alamat = '$alamat', ".
							"email = '$email', ".
							"telepon = '$telepon', ".
							"situs = '$situs' ".
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM perpus_supplier ".
						"WHERE kd = '$kd'");
		}
