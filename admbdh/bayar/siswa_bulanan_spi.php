...
echo '</select>
[<a href="siswa_bulanan_spi_prt.php?tapelkd='.$tapelkd.'&utgl='.$utgl.'&ubln='.$ubln.'&uthn='.$uthn.'"><img src="'.$sumber.'/img/print.gif" border="0" width="16" height="16"></a>]
</td>
</tr>
</table>';
...
if (empty($tapelkd))
	...
	for ($i=1;$i<=$tkhir;$i++)
		...
		$qcc1 = mysql_query("SELECT m_siswa.*, ".
					"siswa_uang_pangkal.kd AS pkd, ".
					"siswa_uang_pangkal.kd_siswa AS kd_siswa ".
					"FROM siswa_uang_pangkal, m_siswa ".
					"WHERE siswa_uang_pangkal.kd_siswa = m_siswa.kd ".
					"AND siswa_uang_pangkal.kd_tapel = '$tapelkd' ".
					"AND siswa_uang_pangkal.nilai <> '' ".
					"AND round(DATE_FORMAT(siswa_uang_pangkal.tgl_bayar, '%d')) = '$i' ".
					"AND round(DATE_FORMAT(siswa_uang_pangkal.tgl_bayar, '%m')) = '$ubln' ".
					"AND round(DATE_FORMAT(siswa_uang_pangkal.tgl_bayar, '%Y')) = '$uthn' ".
		...
		do
			...
			$qjmx = mysql_query("SELECT * FROM siswa_uang_pangkal ".
