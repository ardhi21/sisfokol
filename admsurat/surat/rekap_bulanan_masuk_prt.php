...
//PROSES ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//re-direct print...
$ke = "rekap_bulanan_masuk.php?ubln=$ubln&uthn=$uthn";
...
$qcc = mysql_query("SELECT surat_masuk.*, ".
...
if ($tcc != 0)
	...
	do
		...
		$nomer = $nomer + 1;
		$i_kd = nosql($rcc['kd']);
		$i_no_urut = nosql($rcc['no_urut']);
		$i_no_surat = balikin2($rcc['no_surat']);
		$i_asal = balikin2($rcc['asal']);
		$i_tujuan = balikin2($rcc['tujuan']);
		$i_perihal = balikin2($rcc['perihal']);
		$i_kd_klasifikasi = nosql($rcc['kd_klasifikasi']);
		$i_kd_status = nosql($rcc['kd_status']);
		$i_surat_tgl = nosql($rcc['surat_tgl']);
		$i_tgl_terima = $rcc['tgl_terima'];
		$i_kd_ruang = nosql($rcc['kd_ruang']);
		$i_kd_lemari = nosql($rcc['kd_lemari']);
		$i_kd_rak = nosql($rcc['kd_rak']);
		$i_kd_map = nosql($rcc['kd_map']);
	...
	while ($qcc = mysql_fetch_assoc($rcc));
