...
if ($s == "edit")
	...
	//query
	$qx = mysql_query("SELECT * FROM perpus_kategori ".
						"WHERE kd = '$kdx'");
...
if ($_POST['btnSMP'])
	...
	if ((empty($kode)) OR (empty($kategori)))
		...
		if (empty($s))
			{
			///cek
			$qcc = mysql_query("SELECT * FROM perpus_kategori ".
									"WHERE kode = '$kode'");
			...
			if ($tcc != 0)
				...
				//re-direct
				$pesan = "Kode Kategori : $kode, Sudah Ada. Silahkan Ganti Yang Lain...!!";
				pekem($pesan,$filenya);
				exit();
				}
			else
				{
				//query
				mysql_query("INSERT INTO perpus_kategori(kd, kode, kategori) VALUES ".
								"('$x', '$kode', '$kategori')");
		...
		//jika update
			...
			if (($tcc != 0) AND ($cc_kd != $kd))
				...
				{
				//query
				mysql_query("UPDATE perpus_kategori SET kode = '$kode', ".
								"kategori = '$kategori' ".
								"WHERE kd = '$kd'");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysql_query("DELETE FROM perpus_kategori ".
						"WHERE kd = '$kd'");
		}
...
//query
$q = mysql_query("SELECT * FROM perpus_kategori ".
...
Kode :
<br>
<input name="kode" type="text" value="'.$kode.'" size="3" maxlength="3">
<br>
<br>
Kategori :
<br>
