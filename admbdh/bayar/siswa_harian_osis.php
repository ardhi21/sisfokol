...
//nilai
$filenya = "siswa_harian_osis.php";
$judul = "Laporan Harian : Uang OSIS";
...
if (empty($tapelkd))
	...
	{
	//query
	$qcc = mysql_query("SELECT siswa_uang_osis.*, ".
				"siswa_uang_osis.kd_siswa AS kd_siswa, ".
				"siswa_uang_osis.kd AS pkd, ".
				"m_siswa.* ".
				"FROM siswa_uang_osis, m_siswa ".
				"WHERE siswa_uang_osis.kd_siswa = m_siswa.kd ".
				"AND siswa_uang_osis.nilai <> '' ".
				"AND siswa_uang_osis.kd_tapel = '$tapelkd' ".
				"AND round(DATE_FORMAT(siswa_uang_osis.tgl_bayar, '%d')) = '$utgl' ".
				"AND round(DATE_FORMAT(siswa_uang_osis.tgl_bayar, '%m')) = '$ubln' ".
				"AND round(DATE_FORMAT(siswa_uang_osis.tgl_bayar, '%Y')) = '$uthn' ".
	...
	if ($tcc != 0)
		{
		echo '<br>
		[<a href="siswa_harian_osis_prt.php?tapelkd='.$tapelkd.'&utgl='.$utgl.'&ubln='.$ubln.'&uthn='.$uthn.'"><img src="'.$sumber.'/img/print.gif" border="0" width="16" height="16"></a>]
		...
		do
			...
			//jumlah bayar
			$qjmx = mysql_query("SELECT * FROM siswa_uang_osis ".
						"WHERE nilai <> '' ".
